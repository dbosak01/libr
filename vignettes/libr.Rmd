...
third parameter is the engine to use to read/write the data.  
...
as you would in SAS®.  Note that you can also work with individual variables
on the datasets using dollar sign ("$") syntax.  
```{r eval=FALSE, echo=TRUE} 
...
The data will be available in the workspace for the length of your session.
If you wish to unload it from the workspace, call the `lib_unload()` 
function.  See the `lib_load()` and `lib_unload()` documentation for 
additional information.  

For more library
functions, see the articles on [Basic Library Operations](libr-basics.html)
...
curly braces. You can put a variety of conditional statements and assignments
...
`identical()` function is, when comparing vectors and datasets, it will compare
everything, including attributes.  
...
can be illustrated by comparing these methods side-by-side on the same
input data.
```{r eval=FALSE, echo=TRUE}
# Set up input data
v1 <- c(1, 2, 3)
...
Examine the different results returned by the three different comparison 
